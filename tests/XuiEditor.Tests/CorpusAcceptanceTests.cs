...
{
    private const string CorpusRootEnvironmentVariable =
        "XUI_EDITOR_TEST_CORPUS_ROOT";

    [TestMethod]
    public async Task RepresentativeStockDocumentsParseAndEvaluate()
    {
        string extractedRoot = RequireCorpus();
    ...
    private static string RequireCorpus()
    {
        ...
        if (string.IsNullOrWhiteSpace(extractedRoot) ||
            !Directory.Exists(extractedRoot))
        {
            Assert.Inconclusive(
                $"Set {CorpusRootEnvironmentVariable} to an extracted Dying Light data root to run corpus acceptance tests.");
        }

        return Path.GetFullPath(extractedRoot);
    }
}
